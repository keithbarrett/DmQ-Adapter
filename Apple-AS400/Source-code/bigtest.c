...
/* NOTE:    On AS/400s, this program needs the "AS400" symbol definition */
...
#include <descrip>                      /* Character descriptor macros      */
...
static char             put_buffer[BUF_SIZE];   /* Message buf. +1 null byte*/
...
static char             get_buffer[BUF_SIZE];   /* Message buf. +1 null byte*/
...
char **argv;
{
   ...
   printf("\n\nSIMPLE_CLIENT - sample client program\n");
...
                        
   ...
   /* a response from the server. Quit when user presses <CTRL-Z>.  */

   while (1==1)
   {
      /* Clear type-ahead buffer, then prompt for and read the message. */
      /* If <CTRL-Z> pressed (EOF), we are done, break out of the loop. */

      fflush(stdin);
      printf("\n\nPress ENTER to send %d byte message or type EXIT to exit\n>",
	BUF_SIZE);
      if (gets(put_buffer) == '\0') break;
      if (strncmp(put_buffer, "EXIT", 4) == 0) break;
      if (strncmp(put_buffer, "exit", 4) == 0) break;

    for (i=0, j = 65; i < BUF_SIZE; ++i)
    {
      put_buffer[i] = j++;
      if (j > 90) j = 65;
    }
